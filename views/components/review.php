...
$sql = "SELECT article_id,title,submitted_at,c.category AS category_name FROM `article` JOIN category c ON `article`.category = c.id WHERE `status` = 'review' AND article_id IN (SELECT article_id FROM reviews WHERE reviewer_id = :id)  ORDER BY `submitted_at` DESC;";
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-primary"
                                        onclick="window.location.href='?page=add_review&id=<?php echo $article['article_id'] ?>'">Make
                                        a Review</button>
                                </td>
                            </tr>
                        <?php } ?>
                    ...
                    <tbody>
                        <?php foreach ($inarticles as $article) { ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-primary"
                                        onclick="window.location.href='?page=in_review&id=<?php echo $article['article_id'] ?>'">Make
                                        a Review</button>
                                </td>
                            </tr>
                        <?php } ?>
